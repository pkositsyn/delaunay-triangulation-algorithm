...
Для начала покажем, что время, суммарно затраченное на поиск видимых граней, есть O( n ). Заметим, что на каждой итерации время, затраченное на поиск новой видимой грани, составляет O( 1 ), и после итерации все видимые грани попадают внутрь МВО, то есть мы их больше никогда не найдем. Также мы добавляем в МВО новые 2 ребра. Таким образом, всего в меняющейся на протяжении алгоритма МВО побывает не более 2 * n ребер, значит, и различных видимых ребер будет не более 2 * n, то есть на их поиск мы затратим время O( n ).


Далее, известно, что на многих распространенных распределениях среднее время, затраченное на один рекурсивный вызов из шага 4, составляет O( 1 ), то есть суммарно снова получается O( n ) [ссылка на источник с оценкой времени].
